...
import type { AnyEntity, Dictionary, EntityMetadata, EntityProperty, EntityValue, IHydrator , EntityKey } from '../typings';
import { EagerProps, EntityRepositoryType, HiddenProps, OptionalProps, PrimaryKeyProp } from '../typings';
...
export class EntityHelper {
  ...
  static propagate<T extends object>(meta: EntityMetadata<T>, entity: T, owner: T, prop: EntityProperty<T>, value?: T[keyof T & string], old?: T): void {
    ...
    for (const prop2 of inverseProps) {
      ...
      if (prop.kind === ReferenceKind.ONE_TO_ONE && entity && helper(entity).__initialized) {
        if (
